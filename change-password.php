...
if (Login::isLoggedIn()) {
    if (isset($_POST['changepassword'])) {
        ...
        if (password_verify($oldpassword, DB::query('SELECT password FROM users WHERE id=:userid', array(':userid'=>$userid))[0]['password'])) {
            // the above line is to check if the user has entered the correct password.
    ...
    //This is for the part - Forgot Password !
    if (isset($_GET['token'])) {
        $token = $_GET['token'];
            ...
            if (isset($_POST['changepassword'])) {
                ...
                if ($newpassword == $newpasswordrepeat) {
                    if (strlen($newpassword) >= 6 && strlen($newpassword) <= 60) {
                        DB::query('UPDATE users SET password=:newpassword WHERE id=:userid', array(':newpassword'=>password_hash($newpassword, PASSWORD_BCRYPT), ':userid'=>$userid));
                        echo 'Password changed successfully!';
                        // after using the token, we need to delete it.
...
<form action="<?php if (!$tokenIsValid) { echo 'change-password.php'; } else { echo 'change-password.php?token='.$token.''; } ?>" method="post">
    <!--we have added the php tag here so that we can remove this input field if there is a case of forgot password-->
