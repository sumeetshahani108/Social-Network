...
class Post {
        ...
        if ($loggedInUserId == $profileUserId) {
            if (count(Notify::createNotify($postbody)) != 0) {
                foreach (Notify::createNotify($postbody) as $key => $n) {
                    //$s sends the notification
                    $s = $loggedInUserId;
                    // in $r we store the id to which we have to send notifications i.e it receivesx
                    $r = DB::query('SELECT id FROM users WHERE username=:username', array(':username'=>$key))[0]['id'];
                    // this states that as long as the user exists
                    if ($r != 0) {
    ...
    public static function likePost($postId, $likerId) {
        //below we check if the user has already liked the post
    ...
    public static function displayPosts($userid, $username, $loggedInUserId) {
        ...
        foreach($dbposts as $p) {
            if (!DB::query('SELECT post_id FROM post_likes WHERE post_id=:postid AND user_id=:userid', array(':postid'=>$p['id'], ':userid'=>$loggedInUserId))) {
                ...
                // so that the person can delete his own posts
