...
$verified = False;
$isFollowing = False;
//verified is the concept like - if one user is verified and it follows another person, then it also becomes verified.
if (isset($_GET['username'])) {
    if (DB::query('SELECT username FROM users WHERE username=:username', array(':username'=>$_GET['username']))) {
        $username = DB::query('SELECT username FROM users WHERE username=:username', array(':username'=>$_GET['username']))[0]['username'];
        $userid = DB::query('SELECT id FROM users WHERE username=:username', array(':username'=>$_GET['username']))[0]['id'];
        //get whether the user is verified or not.
        ...
        if (isset($_POST['follow'])) {
            ...
            if ($userid != $followerid) {
                //below we check if the user follows the profile or not.
                if (!DB::query('SELECT follower_id FROM followers WHERE user_id=:userid AND follower_id=:followerid', array(':userid'=>$userid, ':followerid'=>$followerid))) {
                    if ($followerid == 6) {
                        //verified is the concept like - if one user is verified and it follows another person, then it also becomes verified.
                        DB::query('UPDATE users SET verified=1 WHERE id=:userid', array(':userid'=>$userid));
                    }
                    //we insert a new follow for tje user
                    DB::query('INSERT INTO followers VALUES (\'\', :userid, :followerid)', array(':userid'=>$userid, ':followerid'=>$followerid));
                } else {
                    echo 'Already following!';
                }
                $isFollowing = True;
                //for the button to become follow/unfollow
            }
        }

        // the code for the unfollow button
        ...
        //For the post !
        // when posting the body of the post - use htmlspecialchars();
        if (isset($_POST['post'])) {
            // the below code means that the user does not want to upload a file
        ...
        //this part is for the like post
